...
def universal_target_attack_moo(model, dataset_loader, target_class, args, log_rows):
    ...
    def one_restart(restart_idx):
        ...
        for step_index in iterator:
            ...
            mask = (y != target_class)
            if (~mask).all():
                # entire batch is target; skip to next batch draw
                continue
            inp = inp[mask]
            y = y[mask]
            # build target vector for this filtered batch
            ...
            def eot_apply(x, repeats=args.eot_samples):
                ...
                for _r in range(repeats):
                    z = x
                    # DI: with prob di_rate, randomly resize+centerpad to original
                    if args.di_rate > 0.0 and torch.rand(1).item() < args.di_rate:
                        # random scale around 1.0 within di_scale
                    ...
                    # add delta (broadcast) then small gaussian noise
                    z = z + delta
                    if args.eot_noise > 0:
            ...
            if step_index < args.asr_warmup:
                ...
                # kappa ramp: gradually increase to kappa_max
                if args.kappa_max > 0:
            ...
            # Uniform curriculum weights (ASR-heavy early)
            if args.curriculum:
            ...
            with torch.no_grad():
                ...
                if asr_val > best_score:
                    best_score = asr_val
                    best_delta = delta.detach().clone()

            if (step_index + 1) % args.print_every == 0:
                iterator.set_description(
                    f"[T{target_class}] step {step_index+1}/{args.num_steps} | ASR(batch) {100*asr_batch:5.2f}% | ASR(val) {100*asr_val:5.2f}% | sat {100*sat:4.1f}% | margin {m:.4f} | gnorm {gnorm:.3e}"
                )
...
#         # optional tiny FGSM warm-start (made gentler)
#         def fgsm_kick(inp, y):
#             delta_ws = delta.detach().clone().requires_grad_(True)
#             x_ws = (inp + delta_ws).clamp(0, 1)
#             logits_ws = model(x_ws)
...
#             g_delta = torch.autograd.grad(loss_ws, delta_ws)[0]
#             with torch.no_grad():
#                 # gentler kick: eps/16 per kick (so 4 kicks = eps/4)
#                 step = args.eps * max(0.0625, 0.0625)  # eps * 1/16
#                 delta.add_(step * g_delta.sign())
#                 delta.copy_(project_delta(delta, args.eps, args.constraint))
...
#             # Regularizers (we compute them on full-res delta; acceptable for CIFAR)
#             L_spatial = total_variation(delta)
#             L_spectral = fft_high_energy(delta, frac=args.fft_frac)
#             L_clean = delta.pow(2).mean()
...
#                 else:
#                     # For L2 use simple squared soft-hinge
...
#             # Curriculum weights (ASR stays heavier towards the end than before)
#             if args.curriculum:
#                 alpha = float(step_index) / float(max(1, args.num_steps - 1))
#                 # keep ASR heavier (0.90 -> 0.70), increase regularizers modestly
#                 w_asr = 0.90 - 0.20 * alpha   # 0.90 -> 0.70
#                 w_tv  = 0.05 + 0.12 * alpha   # 0.05 -> 0.17
#                 w_sp  = 0.03 + 0.07 * alpha   # 0.03 -> 0.10
#                 w_l2  = 0.02 + 0.06 * alpha   # 0.02 -> 0.08
#             else:
#                 w_asr, w_tv, w_sp, w_l2 = 0.70, 0.12, 0.10, 0.08

#             # Build weighted & EMA-normalized loss list
#             L_list = [
#                 w_asr * (L_asr_eff  / (ema_asr.value   + 1e-8)),
#                 w_tv  * (L_spatial  / (ema_spa.value   + 1e-8)),
#                 w_sp  * (L_spectral / (ema_spec.value  + 1e-8)),
#                 w_l2  * (L_clean    / (ema_clean.value + 1e-8)),
#             ]
#             # Add small hinge clip penalty (not EMA-normalized) to discourage boundary saturation
#             if args.lambda_clip > 0.0:
...
#             # Scheduler / warmup + mid-run anneal for fine-tuning
#             if scheduler is not None:
#                 scheduler.step()
#             else:
#                 if args.warmup > 0 and step_index < args.warmup:
...
#                         pg['lr'] = warm_lr
#                 else:
#                     # after warmup, optionally anneal at mid-run to let optimizer fine-tune
...
#     parser.add_argument('--kappa_max', default=0.5, type=float, help="max kappa for ramp")
#     parser.add_argument('--kappa_ramp_frac', default=0.4, type=float, help="fraction of num_steps used for kappa ramp")
#     parser.add_argument('--eot_samples', default=1, type=int)  # start with 1 for stability, increase later
#     parser.add_argument('--eot_noise', default=0.001, type=float)
#     parser.add_argument('--fft_frac', default=0.5, type=float)
...
#     # Loss / curriculum / PCGrad
#     parser.add_argument('--asr_loss', default='ce', choices=['cw','ce'])  # default to CE for stability
#     parser.add_argument('--curriculum', action='store_true')
#     parser.add_argument('--pcgrad', action='store_true')
...
#     parser.add_argument('--lr_schedule', default='constant', choices=['cosine','constant'])
#     parser.add_argument('--warmup', default=100, type=int)
#     parser.add_argument('--asr_warmup', default=0, type=int)  # start with CE, no CW warmup
#     parser.add_argument('--fgsm_warmstart', default=2, type=int)
#     parser.add_argument('--di_rate', default=0.0, type=float, help="probability of applying DI on each EOT sample")
#     parser.add_argument('--di_scale', default=0.08, type=float, help="scale variation for DI (fraction)")
#     parser.add_argument('--print_every', default=50, type=int)
...
#         args.eps = args.eps / 255.0
#         if args.step_size is None:
#             # choose a default that favors stealth: eps/4
