...
def _proj_simplex(v: torch.Tensor) -> torch.Tensor:
    ...
    rho = torch.max(ind[cond]) - 1  # index in 0-based
...
def universal_target_attack_moo(model, dataset_loader, target_class, args, log_rows):
    ...
    def one_restart(restart_idx):
        ...
        for step_index in iterator:
            ...
            # optional clip hinge penalty (not EMA-normalized)
            ...
            use_mgda_now = bool(args.mgda) and (step_index >= args.asr_warmup)
            use_pcgrad_now = (not use_mgda_now) and bool(args.pcgrad) and (step_index >= args.asr_warmup)
            ...
            if use_mgda_now:
                ...
                # solve small QP over alphas
                device_alpha = grads[0].device if len(grads) > 0 else device
                alpha = solve_mgda_coeffs(grads, iters=args.mgda_iters, lr=args.mgda_lr, device=device_alpha)
                # combine gradients
                g_sum = None
                for a, g in zip(alpha, grads):
                    if g_sum is None:
                        g_sum = (a.to(g.device) * g)
                    else:
                        g_sum = g_sum + (a.to(g.device) * g)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mgda', action='store_true', help="use MGDA solver to combine gradients (takes precedence over --pcgrad)")
